...
class PushUpCounter(private val strictness: Float = 0.3f) : ExerciseCounter {
    ...
    // ── Phase 5.1 — false-positive guards ────────────────────────────────────
    /** Minimum number of the 12 tracked joints that must exceed CONFIDENCE_THRESHOLD. */
    private val MIN_VISIBLE_KEYPOINTS = 6
    private val CONFIDENCE_THRESHOLD  = 0.5f

    /**
     * Consecutive frames a proposed transition must persist before commit.
     * At ~30 fps this adds ≈100 ms but eliminates single-frame noise spikes.
     */
    ...
    override fun process(frame: LandmarkFrame): Analysis {
        // Gate 1: existing validity check (5 key joints at 0.5f)
        if (!frame.isValid) {
            pendingPhase  = null
            pendingFrames = 0
        ...
        // Gate 2 (Phase 5.1): require MIN_VISIBLE_KEYPOINTS confident joints
    ...
    /**
     * Propose a transition to [proposedPhase].  Commits only after
     * TRANSITION_FRAMES_REQUIRED consecutive frames propose the same destination.
     * Returns true on the frame the transition commits.
     */
